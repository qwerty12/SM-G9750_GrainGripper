...
make -j$PROC -C $(pwd) O=$(pwd)/out $KERNEL_MAKE_ENV ARCH=arm64 CROSS_COMPILE=$BUILD_CROSS_COMPILE REAL_CC=$KERNEL_LLVM_BIN CFP_CC=$KERNEL_LLVM_CFP CLANG_TRIPLE=$CLANG_TRIPLE beyond2qlte_chn_open_defconfig
make -j$PROC -C $(pwd) O=$(pwd)/out $KERNEL_MAKE_ENV ARCH=arm64 CROSS_COMPILE=$BUILD_CROSS_COMPILE REAL_CC=$KERNEL_LLVM_BIN CFP_CC=$KERNEL_LLVM_CFP CLANG_TRIPLE=$CLANG_TRIPLE
