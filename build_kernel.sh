...
KERNEL_MAKE_ENV=("DTC_EXT=$DTC_EXT" "CONFIG_BUILD_ARM64_DT_OVERLAY=y")
PROC="$(($(nproc) + 1))"
...
"${DTC_EXT}" "$(pwd)/arch/arm64/boot/dts/qcom/03_dtbdump_<e,u_AAy.dts" >> "$(pwd)/out/arch/arm64/boot/Image-dtb" # TODO: shove this into the relevant Makefile
"$(pwd)/create_image.sh"
